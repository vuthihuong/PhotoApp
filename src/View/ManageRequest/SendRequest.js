                ...
                .equalTo(userKey).on('value', (function (snapshot) {
                    if (snapshot.exists()) {
                        this.itemRef.ref('Post').child(dataSnapshot.key).child('StatusParticipateCol').orderByChild('statusAgree')
                            .equalTo('gửi yêu cầu').on('value', (function (snapshotChild) {
    ...
    render() {
        return (
            <ScrollView style={{ flex: 1, backgroundColor: 'white' }}>
                <View style={stylesSendRequest.containerManagCont}>
                                    ...
                                    {(rowData.title === "Tìm mẫu ảnh") ?
                                        <View style={stylesSendRequest.bodyManaCont}>
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('PostDetailModalView',
                                                ...
                                                <Text style={stylesSendRequest.txtManagCont}>{rowData.title} {rowData.boy} {rowData.girl} </Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Địa điểm: {rowData.value}</Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Thời gian từ {rowData.datetime} đến {rowData.datetime}</Text>
                                                <Text style={stylesSendRequest.txtManagContColor}>Bài đăng ngày {rowData.datePostModal} lúc {rowData.timePostModal}</Text>
                                            </TouchableOpacity>
                                        </View> : null}
                                </View>}
                        /> : null}
                                    ...
                                    {(rowData.title === "Tạo sự kiện") ?
                                        <View style={stylesSendRequest.bodyManaCont}>
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('PostDetailEventView',
                                                ...
                                                <Text style={stylesSendRequest.txtManagCont}>{rowData.labelEvent1}{rowData.labelEvent2} </Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Địa điểm: {rowData.addressEvent}</Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Thời gian từ {rowData.datetimeEvent} đến {rowData.datetimeEvent1}</Text>
                                                <Text style={stylesSendRequest.txtManagContColor}>Bài đăng ngày {rowData.datePostEvent} lúc {rowData.timePostEvent}</Text>
                                            </TouchableOpacity>
                                        </View> : null}
                                </View>}
                        /> : null}
                                    ...
                                    {(rowData.title === "Tìm nháy ảnh") ?
                                        <View style={stylesSendRequest.bodyManaCont}>
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate('PostDetailPhotoView',
                                                ...
                                                <Text style={stylesSendRequest.txtManagCont}>{rowData.title} </Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Địa điểm: {rowData.valuePlacePhoto}</Text>
                                                <Text style={stylesSendRequest.txtManagCont}>Thời gian từ {rowData.datetimePhoto} đến {rowData.datetimePhoto1}</Text>
                                                <Text style={stylesSendRequest.txtManagContColor}>Bài đăng ngày {rowData.datePostPhoto} lúc {rowData.timePostPhoto}</Text>
                                            </TouchableOpacity>
                                        </View> : null}
                                </View>}
                        /> : null}
